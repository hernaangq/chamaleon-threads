...
void mode_generate() {
    ...
    for (uint64_t r = 0; r < rounds; r++) {
        ...
        if (cfg.debug) {
            ...
            double mh = count / (now - t0) / 1e6;
            double mb = count * RECORD_SIZE / (now - t0) / 1048576.0;
    ...
    double total_mh = total_records / total_time / 1e6;
    double total_mb = total_records * RECORD_SIZE / total_time / 1048576.0;
...
int main(int argc, char *argv[]) {
    ...
    static struct option long_opts[] = {
        ...
        {"search", 1, 0, 's'}, {"difficulty", 1, 0, 'q'}, {"help", 0, 0, 'h'},
        {0,0,0,0}
    };

    while ((opt = getopt_long(argc, argv, "a:t:i:k:m:f:g:d:p:s:q:h", long_opts, NULL)) != -1) {
        switch (opt) {
